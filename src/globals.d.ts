declare global {
  ...
  function printerr(error: any): void;
  function log(message?: string): void;
  function logError(exception: any, message?: string): void;

  interface Pkg {
    /**
     * Initialize directories and global variables. Must be
     * called before any of other API in Package is used.
     * `params` must be an object with at least the following
     * keys:
     *
     ...
     * single entry point. You must define a main(ARGV) function
     * inside a main.js module in moduledir.
     */
    start(
      options: Pick<Pkg, "name" | "version" | "prefix"> &
        Partial<Pick<Pkg, "libdir">>
     ...
     *   entry points in one package. You must define a
     *   main(ARGV) function inside the passed in module, and
     *   then the launcher would be:
     *
     * @example
     ...
     * datadir + name when installed and './data' when running
     * from the source tree
     */
    pkgdatadir: string;
    /**
     * The directory to look for private typelibs and C
     * libraries; this will be libdir + name when installed and
     * './lib' when running from the source tree
     */
    pkglibdir: string;
    /**
     * The directory to look for JS modules; this will be
     * pkglibdir when installed and './src' when running from the
     * source tree
     */
    moduledir: string;
    /**
     * The directory containing gettext translation files; this
     * will be datadir + '/locale' when installed and './po' in
     * the source tree
     */
    localedir: string;
  }
